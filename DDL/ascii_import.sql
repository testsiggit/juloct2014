...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 16, 2014 at 12:19 PM
...
CREATE TABLE IF NOT EXISTS `da_bottom_hole_location` (
  ...
  `pmt_bhl_lease_direction_1` varchar(20) DEFAULT NULL,
  `pmt_bhl_lease_feet_2` decimal(8,2) DEFAULT NULL,
  `pmt_bhl_lease_directon_2` varchar(20) DEFAULT NULL,
  ...
  `pmt_bhl_surface_lease_direction_1` varchar(20) DEFAULT NULL,
  `pmt_bhl_surface_lease_feet_2` decimal(8,2) DEFAULT NULL,
  `pmt_bhl_surface_lease_direction_2` varchar(20) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `da_field` (
  ...
  KEY `field_application_well_code` (`field_application_well_code`)
...
CREATE TABLE IF NOT EXISTS `da_field_bottom_hole_location` (
  ...
  KEY `fld_bhl_section` (`fld_bhl_section`)
...
CREATE TABLE IF NOT EXISTS `da_free_form_restrictions` (
  ...
  `permit_master_id` int(11) DEFAULT NULL,
  `rrc_tape_record_id` varchar(2) CHARACTER SET latin1 DEFAULT NULL,
  `free_restr_key` varchar(2) CHARACTER SET latin1 DEFAULT NULL,
  `free_restr_type` varchar(2) CHARACTER SET latin1 DEFAULT NULL,
  `free_restr_remark` varchar(2000) CHARACTER SET latin1 DEFAULT NULL,
  `free_restr_flag` varchar(1) CHARACTER SET latin1 DEFAULT NULL,
  `filler` varchar(10) CHARACTER SET latin1 DEFAULT NULL,
  `rrc_tape_filler` varchar(45) CHARACTER SET latin1 DEFAULT NULL,
  PRIMARY KEY (`free_form_restrictions_id`),
  KEY `free form rest FK_idx` (`permit_master_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='08 - DAFRERES - DA FREE-FORM RESTRICTIONS' AUTO_INCREMENT=1 ;
...
CREATE TABLE IF NOT EXISTS `da_permit_master` (
  ...
  `final_approved_date` date DEFAULT NULL COMMENT 'Insert da_pmt_amended_date if it is not null otherwise da_pmt_issued_date',
  ...
  KEY `final_approved_date` (`final_approved_date`),
  KEY `horizontal_well_flag` (`horizontal_well_flag`),
  KEY `api_number` (`api_number`),
  KEY `permit_total_depth` (`permit_total_depth`),
  KEY `type_application` (`type_application`),
  KEY `permit_number` (`permit_number`),
  KEY `permit_sequence_number` (`permit_sequence_number`)
...
CREATE TABLE IF NOT EXISTS `da_processed_files` (
  ...
  `process_status` enum('INPROCESS','COMPLETED') DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `da_root` (
  ...
  KEY `status_number` (`status_number`),
  KEY `status_of_app_flag` (`status_of_app_flag`),
  KEY `status_sequence_number` (`status_sequence_number`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
